...
public class Notenverwaltung {
  ...
  //Feature2: Methode zum aufrufen der Notenlöschen-Methode
  public void noteloeschen(int index, int stelle) {
    fach = faecher.get((index-1));
    fach.loescheNote(stelle);
    speichern();
   }
  //Feature2: Methode zum aufrufen der NotenÄndern-Methode
  public void noteaendern(int index, int stelle, int wert, String leistung, int gewichtung){
    fach = faecher.get((index-1));
    List<Note> notenliste = getNoten(fach.getFachname());
    Note neuenote = notenliste.get(stelle-1);
    neuenote.setNotenpunkte(wert);
    if (leistung.length() > 0) {
    neuenote.setErbrachteLeistung(leistung);
    }
    if (gewichtung > 0) {
      neuenote.setGewichtung(gewichtung);
    }
